...
def test_discovery_empty_config():
    ...
    discovery = ToolDiscovery()
    tools = discovery.discover_all(include_builtin=False, config_file="nonexistent.yaml")
...
def test_discovery_invalid_directory():
    ...
    discovery = ToolDiscovery()
    tools = discovery.discover_all(include_builtin=False, custom_paths=["nonexistent_dir"])

...
from jarvis.tools.base import Tool, ToolParameter, RiskLevel, ToolResult
...
class DummyTool(Tool):
    ...
    def get_parameters(self) -> list[ToolParameter]:
        ...
        return [
            ToolParameter(name="value", type="string", description="Test value")
        ]
...
async def test_tool_availability_documented_behavior():
    ...
    first_response = LLMResponse(
        ...
        tool_calls=[
            ToolCall(name="dummy_tool", arguments={"value": "test"})
        ],
    )
...
async def test_tool_availability_flag_resets_per_request():
    ...
    mock_llm.complete.side_effect = [
        LLMResponse(content="Tool", tool_calls=[ToolCall(name="dummy_tool", arguments={"value": "1"})]),
        LLMResponse(content="Done 1", tool_calls=None),
        LLMResponse(content="Tool", tool_calls=[ToolCall(name="dummy_tool", arguments={"value": "2"})]),

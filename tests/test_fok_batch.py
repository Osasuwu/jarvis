...
# Import functions to test (will work after sys.path setup in conftest or explicit import)
# We'll do inline imports to handle module discovery


def test_build_user_message():
    """Test building the user message for Haiku with query and memory snippets."""
    from scripts.fok_batch import build_user_message
    ...
    msg = build_user_message(query, returned_results)
...
def test_judge_via_haiku_missing_api_key():
    ...
    from scripts.fok_batch import judge_via_haiku

    with patch.dict("os.environ", {"ANTHROPIC_API_KEY": ""}, clear=False):
        result = judge_via_haiku("query", [])
...
def test_judge_via_haiku_json_in_prose():
    """Test JSON extraction from prose response."""
    from scripts.fok_batch import judge_via_haiku
    ...
    # Haiku response with JSON embedded in prose
    prose = f"""
    ...
    with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}):
        with patch("scripts.fok_batch.httpx.Client") as mock_httpx:
            mock_response = Mock()
            mock_response.json.return_value = {"content": [{"text": prose}]}
            mock_httpx.return_value.__enter__.return_value.post.return_value = (
                mock_response
            )

            result = judge_via_haiku("test query", [])
...
def test_judge_via_haiku_missing_httpx():
    ...
    try:
        ...
        assert result["verdict"] == "unknown"
        assert "httpx" in result.get("reason", "").lower()
    finally:
...
def test_try_insert_known_unknown_sufficient_verdict():
    ...
    from scripts.fok_batch import try_insert_known_unknown
    ...
    try_insert_known_unknown(mock_client, event, "test-project")
...
def test_try_insert_known_unknown_above_similarity_threshold():
    ...
    mock_client.table.return_value.select.return_value.limit.return_value.execute.return_value = mock_sim_result
...
def test_write_verdict_to_event():
    ...
    from scripts.fok_batch import write_verdict_to_event

    mock_client = MagicMock()
    event_id = "evt-001"
    verdict = {"verdict": "partial", "confidence": 0.7, "reason": "Some info present."}

    write_verdict_to_event(mock_client, event_id, verdict)
...
def test_write_event_summary():
    ...
    from scripts.fok_batch import write_event
    ...
    write_event(mock_client, summary, "test-project")
...
def test_fetch_events_filters_unfudged():
    ...
    from scripts.fok_batch import fetch_events
    ...
    events = fetch_events(mock_client, 10)

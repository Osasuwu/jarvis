...
- **Issue already carries `agent:dispatch`** → **already dispatched**, treat as a no-op collision: report `#N already dispatched — skipping` and don't re-apply. This is the label-based equivalent of the old queue's unique-index collision check; a GitHub label is naturally idempotent (adding a label the issue already has is a no-op), so there is no race window to design around here the way there was with a queue row.

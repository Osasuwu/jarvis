...
Copy `template.sh` to the target path (default: a scratch path or `scripts/`, per jarvis's scratchpad convention — see the project's CLAUDE.md `Scratchpad Directory`). Replace the example stage with one `stage` per step, in dependency order. Use the library helpers — `stage`, `say`/`step`, `open_url`, `ask`/`ask_secret`, `write_env`, `set_secret`/`set_var`, `pause`/`confirm` — and set `TOTAL_STAGES` to the number of stages you wrote.

...
1. **Pre-compact snapshot** — **do NOT use `memory_recall` for this**: snapshot rows are tagged `session-snapshot`, which `memory_recall` excludes by design (`EXCLUDE_TAGS_FROM_RECALL`, #417) — the query will never return them. Use `memory_list(project="jarvis", type="project")` instead; rows come back newest-first within the type. Pick the first entry whose name starts with `session_snapshot_` (ignore any with `test` in the name) and check its `updated_at` is from *this* session's timeframe. Then `memory_get(name=..., project="jarvis")` to load the full content.

...
1. **Pre-compact snapshot** — **do NOT use `memory_recall` for this**: snapshot rows are tagged `session-snapshot`, which `memory_recall` excludes by design (`EXCLUDE_TAGS_FROM_RECALL`, #417) — the query will never return them.
   - **Preferred — indexed point lookup.** If you know this session's id, fetch the row directly: `memory_get(name="session_snapshot_<session_id>", project="jarvis")`. The id is the `# Session Snapshot — <session_id>` header in the auto-loaded *Pre-Compact Recovery* block (and the `session_id` the SessionStart hook surfaces). This is how `session-context.py` loads it; the lookup hits the unique `(project, name)` B-tree index, so its cost is constant relative to how many snapshot rows have piled up (O(log N) on table size, not O(rows-scanned)).
   - **Fallback — id genuinely unknown.** `memory_list(project="jarvis", type="project", limit=20)`. With `type="project"` specified rows come back newest-first *within that type* — the handler sorts `type` asc then `updated_at` desc, and with a single `type` value applied all returned rows share that type, so the `type` sort is a no-op and the effective order is `updated_at` desc. Do **not** drop the type filter: an unfiltered list interleaves types and the "newest-first" property no longer holds. Pick the first entry whose name starts with `session_snapshot_` (ignore any with `test` in the name). Caveat: `limit=20` silently misses the snapshot if >19 newer `type=project` rows landed since — which is exactly why the exact lookup above is preferred. Then `memory_get(name=..., project="jarvis")` to load the full content.
   - **Freshness check.** An LLM has no reliable session-start timestamp, so don't try to test whether `updated_at` is "from *this* session". Instead apply a concrete cutoff: treat any snapshot whose `updated_at` is **older than 4 hours** as potentially stale and run the content-mismatch check below before trusting it. Note this 4-hour cutoff is for the **direct `memory_get` fetch path only** — if you took the snapshot straight from the auto-loaded *Pre-Compact Recovery* block, `session-context.py` already gated it at `PRE_COMPACT_FRESHNESS_MINUTES` (30 min), so anything surfacing there is fresher than this cutoff by construction.
   - If no snapshot found → this session never compacted. Fine, skip. Conversation alone is enough.
   - If this session **did** compact but no fresh `session_snapshot_*` row exists → the PreCompact hook didn't fire. Known failure mode: rewriting `~/.claude/settings.json` (e.g. `install.ps1 -Apply`) while the desktop app runs disables ALL hooks for every spawned session until app restart (2026-06-12 outage). Check `.claude/session-snapshots/hook.log` (via `Read` or `Bash`) — a missing heartbeat line at compaction time confirms it. Flag in Step 8 output and fall back to conversation only.

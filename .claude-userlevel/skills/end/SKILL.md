...
1. **Pre-compact snapshot** — **do NOT use `memory_recall` for this**: snapshot rows are tagged `session-snapshot`, which `memory_recall` excludes by design (`EXCLUDE_TAGS_FROM_RECALL`, #417) — the query will never return them.
   ...
   - **Fallback — id genuinely unknown.** `memory_list(project="jarvis", type="project", limit=5)`. With `type="project"` specified rows come back newest-first *within that type* — the handler sorts `type` asc then `updated_at` desc, and with a single `type` value applied all returned rows share that type, so the `type` sort is a no-op and the effective order is `updated_at` desc. Do **not** drop the type filter: an unfiltered list interleaves types and the "newest-first" property no longer holds. Pick the first entry whose name starts with `session_snapshot_` (ignore any with `test` in the name). Caveat: `limit=5` silently misses the snapshot if >4 newer `type=project` rows landed since — which is exactly why the exact lookup above is preferred. Then `memory_get(name=..., project="jarvis")` to load the full content.

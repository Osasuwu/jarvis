...
- **Pre-dispatch gate** — readiness check `/delegate` runs *before* spawning any sandcastle subagent for issue #N (issue #642). Four conditions, all required: (1) `sandcastle` label present (explicit AFK-safe classification, not auto-inferred); (2) no `needs-*` label (`needs-grill`, `needs-research`, `needs-prd` etc.); (3) issue body has `## Acceptance criteria` section; (4) issue body references ≥1 decision UUID (`[0-9a-f]{8}-[0-9a-f]{4}-…`). Any failure ⇒ refuse with a message naming the missing element; owner fixes upstream (adds artefacts / unblocks / labels) and retries. Lives in `/delegate` only — interactive `/implement` is *not* gated this way (SOUL.md grill-checkbox is the in-skill backstop when an operator is present). Rationale: headless = no operator to grill against, so AFK-readiness must be verified at dispatch time, not inside the dispatched agent.

...
def _run_git(args: list[str], cwd: Path = ROOT_DIR) -> str:
    result = subprocess.run(
        ["git", *args],
        capture_output=True, text=True, encoding="utf-8",
        errors="replace", cwd=str(cwd), timeout=30,
    )
...
def _run_gh(args: list[str], cwd: Path = ROOT_DIR) -> str:
    result = subprocess.run(
        ["gh", *args],
        capture_output=True, text=True, encoding="utf-8",
        errors="replace", cwd=str(cwd), timeout=30,
    ...
    if auto_applied > 0 and not dry_run:
        ...
        if changed_files:
            ...
            try:
                ...
                _run_git(["stash", "push", "-m", "self-improve-temp"])
                try:

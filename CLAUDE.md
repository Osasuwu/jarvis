...
- **`/status` and `/morning` use anchored routing — bare/unrelated uses of the trigger word do NOT fire them.** Only the exact triggers `статус`/`status`/`статус <repo>` route to `/status`, and only `утро`/`morning`/`доброе утро` route to `/morning`. A sentence that merely contains one of these words — "какой статус у PR #123", "status code 500", "good morning" as a passing greeting, "meeting moved to this morning" — is a normal request answered in-context, never a trigger. The two trigger sets are disjoint by construction — neither intercepts the other's.

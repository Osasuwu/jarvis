...
- Branches from `main`. **PRs are for code, not for discussions.**
  ...
  - **Consolidation / batch PR → enumerate EVERY `Closes #N` it ships** (one line per subsumed issue). GitHub's native linked-issue auto-close only fires for issues named in the PR body; an issue whose work is bundled in but not listed silently stays open (Mode 2 of #948 — PR #900 shipped 5 issues' work, listed 1, left 4 open). When you close a standalone branch PR as "shipped via #X", add the corresponding `Closes #N` to #X's body. No automated backstop recovers this — the closing linkage exists only if the author writes it. (Mode 1 — a listed `Closes #N` that still didn't fire — was a bot-merge-identity bug, fixed by the App-token merge in `auto-merge-enable.yml`; see memory `github_token_merge_suppresses_native_issue_autoclose`.)

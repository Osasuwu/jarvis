...
Has its own gating: drafts welcome; final send stays with the principal until the digital-twin mode is mature — see the user-level `weekly-release` skill for a concrete example of this boundary enforced in a skill. Uses all five axes, but with judgment **calibrated to** the principal's voice rather than Jarvis's own — through accumulated examples and explicit style memories, not model fine-tuning.

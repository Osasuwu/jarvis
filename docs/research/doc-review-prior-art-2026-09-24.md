...
| # | Изменение | Статус |
|---|---|---|
| 1 | Скоринг печатает catch@3 (объединение: поймано хоть в одном прогоне) и catch^3 (во всех) рядом с per-run | catch@3 уже был в плане (Target: union рядом с медианой). Новое только catch^3 и счётчик попаданий 0–3 на запись — внесены через `/grill` 2026-09-24 (jarvis-oss#152) |
| 2 | Проход, сливающий находки k прогонов, как кандидат | Внесено (jarvis-oss#152) только как кандидат через dev gain rule и cost caps; решать о нём по результату на test запрещено. Потерю при слиянии можно померить без новых прогонов: слить 3 готовых dev-отчёта промптом, зафиксированным заранее |

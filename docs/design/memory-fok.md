...
**Status:** 5.3-α design (this doc) — six decisions presented for owner review **before** any 5.3-β code.
**Closes:** #420.
**Parent epic:** #185.
...
## 6. Open questions (require owner answer before 5.3-β)

1. **Hook-level emit (D2-bis)** — confirm we want UserPromptSubmit recalls in the FOK pipeline. Cost is fine; the question is whether brief-mode previews are *worth* judging or whether their sufficiency is the agent's call (since the agent decides whether to `memory_get` for full content).
2. **Sample budget** — 50 judgments/day cap. Is this enough? With hook emit, raw event volume could be 100–300/day. Prioritization rules (§D2) decide which 50 land. Acceptable?
3. **30-min window for outcome linkage (D6)** — sane default? Tasks that span hours (e.g. /research, multi-step /implement) won't link cleanly. Alternative: link via `decision_made.payload.recall_event_ids` if we extend `record_decision` to capture that explicitly. More work but more correct.
4. **Backward-compat mirror to `events.payload`** — keep it (D4) or skip and update `fok-batch.py` to write only the new table? Simpler if we don't mirror; but then any external consumer of `events.payload.fok_verdict` breaks silently.
5. **Inline `known_unknowns` removal** — D5 says yes. Risk: if `fok-batch.py` falls behind for any reason, gaps stop accumulating until it catches up. Acceptable if cron is reliable; alternatively keep one inline path at `top_sim < 0.3` (only the egregious misses) as a backstop.
...
## 8. Decision summary (for owner sign-off)

| # | Topic | Choice | vs. #250 |
|---|---|---|---|
| D1 | Trigger | Batch consumer of `events`, scheduled cron | reaffirm |
| D2 | Inputs | `{query, top-5 (id, content<2KB, sim)}` + sample 50/day, prioritize low-sim/low-count | reaffirm + scope: hook recalls included |
| D3 | Model | `claude-haiku-4-5-20251001` | reaffirm spec, revise shipped code |
| D4 | Storage | New `fok_judgments` table, `events.payload` becomes deprecated mirror | revise |
...
**5.3-β starts only after each row is approved or revised in PR review.**

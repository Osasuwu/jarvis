...
Same shape as the consolidation job (below). `scripts/evolve-run.py` runs weekly, Sundays 11:00 Astana, as `memory-evolve-weekly`. Emits one `evolve_run` event per run (`source='scheduled_task'`, severity `info` on clean / `medium` when `queued_pending >= 1` / `high` on subprocess or parse failure). When `needs_review=true` the task session surfaces a review chip; the queued EVOLVE plans show up in `memory_review_queue` with `decision='EVOLVE' AND status='pending'`. Review is **CLI-less today** — `consolidation-review.py` (#229) only dispatches on MERGE/SUPERSEDE_CONSOLIDATION, so EVOLVE plans are inspected via direct SQL (`select id, evolution_payload from memory_review_queue where decision='EVOLVE' and status='pending'`) and approved/rejected by updating `status` + invoking `apply_evolution_plan(...)` / `rollback_evolution(...)` manually. Extending the review CLI to include EVOLVE is tracked as #235, not part of 5.2-γ. Disable: `mcp__scheduled-tasks__update_scheduled_task(taskId='memory-evolve-weekly', enabled=false)`. Manual smoke-test: `python scripts/evolve-run.py --dry-run --no-save-memory --limit 2` from repo root (does not pass `--apply`, still emits an event).

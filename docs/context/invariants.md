...
- **Sending as the owner isn't autonomous** until "digital twin" ships — drafts OK, send stays with the owner. Foreign-owner writes: separate axis, see `config/repos.conf`.
...
- **`mcp-memory/server.py`, `.mcp.json`, Supabase schema are shared surfaces** — consumers sit outside this repo (redrobot, future jarvis-oss operators); breakage is invisible from inside it. Verify consumers before pushing.

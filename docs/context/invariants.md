...
- **Post-compact context replay is harness-level, not jarvis-influenceable** — Claude Code re-injects invoked skill bodies (capped 5,000 tok/skill, 25,000 total, truncated from file start) and recently-read tool-result tail by design, for behavioral continuity. Neither of jarvis's own compaction hooks can touch this: `pre-compact-backup.py` (PreCompact) is a pure side-effect snapshot backup, and `session-context.py`'s `matcher:compact` path is strictly additive (prepends a memory pointer). No settings key, hook, or env var filters or suppresses the replay — the only upstream lever (a `PostCompact` hook) is an open, unimplemented feature request (`anthropics/claude-code#14258`). Don't re-litigate this as a jarvis bug (#1451, decision `75e9aabd`).

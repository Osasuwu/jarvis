...
| merge_candidates | Duplicate coverage detected | passed | 0 MERGE candidates found (previously 1; revised to KEEP after cross-report resolution — circular-import detection cannot be replaced) |
| rewrite_candidates | Medium-value tests needing rewrite | passed | 6 REWRITE candidates identified |
| keep_candidates | High-value tests | passed | 6 KEEP candidates identified |
...
| LOW | tests/test_memory_server_script_launch.py | 66 lines, single test for script launch regression guard — subprocess approach serves unique circular-import detection purpose | Value | KEEP standalone — do not merge; import-based replacement would lose circular-import detection (cross-report resolution with ln-635) | S |

## Detailed Scores

| Test File | Lines | Impact (1-5) | Probability (1-5) | Score | Decision | Notes |
|-----------|-------|-------------|-------------------|-------|----------|-------|
| test_memory_calibration.py | 143 | 4 | 3 | 12 | KEEP | 143 lines, well-scoped, LOW finding — no actionable issue; Findings table verdict is KEEP |
...
| test_memory_server_script_launch.py | 66 | 3 | 3 | 9 | KEEP | Keep standalone — subprocess serves unique circular-import detection purpose; merging or replacing with import-based test would lose this capability (cross-report resolution: ln-635 Findings updated to match) |
...
| test_evolve_neighbors.py | 753 | 4 | 3 | 12 | KEEP | Findings table says KEEP — "valuable defensive layer for Haiku output parsing"; MEDIUM notes high maintenance but recommendation is Keep, not Rewrite |
...
| test_session_context_recovery.py | 445 | 4 | 3 | 12 | KEEP | Findings table says KEEP — "no actionable issue"; MEDIUM finding is informational, not an improvement directive |
...
- **7 KEEP** — core infrastructure tests (memory server, episode extractor, recall orchestrator) plus tests with no actionable issue (memory calibration, evolve neighbors, session context recovery) plus script launch test (circular-import detection, irreplaceable)
- **0 MERGE** — revised from 1; script launch kept standalone (cross-report resolution with ln-635)
- **6 REWRITE** — medium-value tests that could benefit from fixture extraction, boilerplate reduction, or scope trimming
